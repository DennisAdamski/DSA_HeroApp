...
AdvancementOption? resolveAdvancementManeuver(
    AdvancementContext context, String targetId) {
  final parts = targetId.split('::');
  if (parts.length > 2) return null;
  final def = context.catalog.maneuvers
      .where((def) => def.id == parts.first).firstOrNull;
  if (def == null) return null;
  final eligible = _eligibleTalents(context.hero, context.catalog, def);
  final selected = parts.length == 2
      ? eligible.where((talent) => talent.id == parts[1]).firstOrNull : null;
  if (parts.length == 2 && (!def.mussSeparatErlerntWerden || selected == null)) {
    return null;
  }
  ...
  final requirements = def.voraussetzungenStruktur.isEmpty &&
      def.voraussetzungen.trim().isNotEmpty
      ? [SpecialAbilityRequirement(art: RequirementArt.hinweis,
          text: def.voraussetzungen)] : def.voraussetzungenStruktur;
  final bound = requirements.map((r) => _bindTalent(r, selected?.name ?? ''));
  return AdvancementOption(kind: AdvancementKind.maneuver, targetId: targetId,
    label: selected == null ? def.name : '${def.name} (${selected.name})',
    ability: def, isOwned: owned, unavailableReason: unavailable,
...
List<TalentDef> _eligibleTalents(
    HeroSheet hero, RulesCatalog catalog, ManeuverDef def) {
  return catalog.talents.where((talent) {
    final value = hero.talents[talent.id]?.talentValue ?? -1;
    return talent.active && value >= 0 &&
        (def.giltFuerTalentTyp.isEmpty || talent.type == def.giltFuerTalentTyp) &&
        (def.nurFuerTalente.isEmpty || def.nurFuerTalente.contains(talent.id));
  }).toList(growable: false);
}

// Der Katalog kann denselben Vertrag für jedes Fernkampftalent ausdrücken.
SpecialAbilityRequirement _bindTalent(SpecialAbilityRequirement r, String name) {
  final json = r.toJson();
  if (r.name.contains(r'$talent')) json['name'] = r.name.replaceAll(r'$talent', name);
  if (r.bedingungen.isNotEmpty) {
    json['bedingungen'] = r.bedingungen
        .map((child) => _bindTalent(child, name).toJson()).toList();
  }
