  ...
  Map<String, int> levels(List<String> ids) {
    ...
    while (pending.isNotEmpty) {
      ...
      for (final id in pending.toList()) {
        final parents = edges.where(
          (e) => e.to == id && included.contains(e.from),
        );
...
AdvancementSkillTree buildAdvancementSkillTree({
  ...
  for (final option in abilityOptions) {
    ...
    nodes[id] = SkillTreeNode(
      id: id,
      label: option.label,
      category: _category(option.kind),
      option: option,
...
String _requirementNode(
  ...
  final group =
      requirement.art == RequirementArt.oderGruppe ||
      requirement.art == RequirementArt.undGruppe;
  if (!group) {
    final matches = skills.where(
      (node) => _matchesReference(node, requirement),
    );
  ...
  nodes[id] = SkillTreeNode(
    ...
    label: label,
    category: category,
    status: check.istOffen
        ? SkillTreeStatus.blocked
        : check.status == RequirementStatus.hinweis
        ? SkillTreeStatus.review
        : SkillTreeStatus.available,
  );
  for (var i = 0; i < check.teilergebnisse.length; i++) {
    final parent = _requirementNode(
      check.teilergebnisse[i],
      '$id:$i',
      category,
      skills,
      nodes,
...
bool _matchesReference(SkillTreeNode node, SpecialAbilityRequirement r) {
  ...
  if (r.art != RequirementArt.manoever &&
      r.art != RequirementArt.sonderfertigkeit) {
    return false;
  }
...
SkillTreeStatus _status(AdvancementOption option, bool planned, int ap) {
  ...
  if (option.apCost == null || option.requirements.any(_needsReview)) {
...
bool _needsReview(RequirementCheckResult check) =>
    check.status == RequirementStatus.hinweis ||
    check.teilergebnisse.any(_needsReview);
