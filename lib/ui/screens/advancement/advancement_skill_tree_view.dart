...
class AdvancementSkillTreeView extends StatefulWidget {
  ...
  const AdvancementSkillTreeView({super.key, required this.session,
    required this.options, required this.query, this.onPlan});
  ...
  @override
  State<AdvancementSkillTreeView> createState() => _AdvancementSkillTreeViewState();
}
...
class _AdvancementSkillTreeViewState extends State<AdvancementSkillTreeView> {
  ...
  @override
  void initState() { super.initState(); _rebuildGraph(); }

  @override
  void didUpdateWidget(covariant AdvancementSkillTreeView oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.options != widget.options || oldWidget.session != widget.session) {
      _rebuildGraph();
    }
  }
  ...
  void _rebuildGraph() {
    _graph = buildAdvancementSkillTree(options: widget.options,
  ...
  Widget build(BuildContext context) {
    final branches = _graph.components(query: widget.query, category: _category);
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      SingleChildScrollView(scrollDirection: Axis.horizontal,
        padding: const EdgeInsets.symmetric(horizontal: 16),
        child: Row(children: [
          for (final category in [null, 'Kampf', 'Allgemein', 'Magie', 'Karma'])
            Padding(padding: const EdgeInsets.only(right: 8), child: ChoiceChip(
              label: Text(category ?? 'Alle'), selected: _category == category,
          ...
          if (index == 0) {
            return Padding(padding: const EdgeInsets.only(bottom: 16),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Fähigkeitenbaum', style: Theme.of(context).textTheme.titleLarge),
                const SizedBox(height: 6),
                const Text('Von links nach rechts entwickeln. Knoten öffnen für '
                ...
                if (branches.isEmpty) ...[
                  ...
                  const Text('Keine passenden Fähigkeiten. Suche oder Bereich ändern.'),
                ],
              ]));
          }
          final ids = branches[index - 1];
          return Padding(padding: const EdgeInsets.only(bottom: 20),
            child: SkillTreeBranch(graph: _graph, ids: ids,
  ...
  Future<void> _openDetails(SkillTreeNode node) async {
    ...
    try {
      final plan = await showAdaptiveInputDialog<bool>(context: context,
        builder: (context) => AdaptiveInputDialog(title: option.label,
          content: AdvancementOptionCard(option: option,
            expandRequirements: true,
            planned: node.status == SkillTreeStatus.planned,
            onPlan: widget.onPlan == null ? null : () => Navigator.of(context).pop(true)),
