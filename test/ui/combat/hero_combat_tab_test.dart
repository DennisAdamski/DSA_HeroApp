...
void main() {
  ...
  testWidgets('abgebrochener Erwerb laesst die AP unveraendert', (
    ...
    expect(hero.combatConfig.specialRules.schnellziehen, isFalse);
    expect(hero.apSpent, 500);
  });
  ...
  testWidgets('combat techniques support manual correction without AP', (
    ...
    final repo = FakeRepository(
      heroes: [
        buildHero(
          talents: const <String, HeroTalentEntry>{
            'tal_nah': HeroTalentEntry(talentValue: 5, atValue: 3, paValue: 2),
          },
        ).copyWith(apAvailable: 500),
      ],
      states: {
        'demo': const HeroState(
          currentLep: 10,
  ...
  testWidgets('Kampftechnik-Spezialisierung zieht AP ab', (tester) async {
    final repo = FakeRepository(
      heroes: [
        buildHero(
          talents: const <String, HeroTalentEntry>{
    ...
    expect(hero.talents['tal_nah']!.combatSpecializations, const <String>[
      'Schwert',
    ]);
  ...
  testWidgets(
    'artifact summary stays hidden without active artifact descriptions',
    (tester) async {
      final repo = FakeRepository(
        heroes: [
          buildHero(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(
          const ValueKey<String>('combat-artifact-summary-group-mainhand'),
        ),
        findsOneWidget,
      );
      expect(
        find.byKey(
          const ValueKey<String>('combat-artifact-summary-group-offhand'),
        ),
        findsOneWidget,
      );
      expect(
        find.byKey(
          const ValueKey<String>('combat-artifact-summary-group-armor'),
        ),
        findsOneWidget,
      );
      expect(
        find.descendant(
          of: artifactCard,
          matching: find.text('Gebundener Dschinn'),
        ),
        findsOneWidget,
      );
      expect(
  ...
  testWidgets(
    'artifact summary is placed between offhand and calculation details',
    (tester) async {
      final repo = FakeRepository(
        heroes: [
          buildHero(
