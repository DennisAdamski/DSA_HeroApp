...
const _catalog = RulesCatalog(
  ...
  maneuvers: [
    ManeuverDef(
      ...
      name: 'Wuchtschlag',
      kosten: '100 AP',
      erklarung: 'Gezielter Kraftaufwand',
      erklarungLang: 'Vollständiger Regeltext zum Wuchtschlag.',
      voraussetzungen: 'Ein passendes Kampftalent',
      verbreitung: 'Weit verbreitet',
      quelle: 'Testregelwerk',
      seite: '42',
      erschwernis: '+4',
    ),
    ManeuverDef(
...
void main() {
  Future<ProviderContainer> open(WidgetTester tester, Size size) async {
    ...
    container
        .read(advancementSessionProvider(_hero.id).notifier)
        .start(hero: _hero, catalog: _catalog);
    await tester.pumpWidget(
      UncontrolledProviderScope(
        container: container,
        child: const MaterialApp(
          home: Scaffold(body: AdvancementCatalog(heroId: 'tree')),
        ),
      ),
    );
  ...
  testWidgets('Manöver direkt im Baum erwerben und Folgepfad freischalten', (
    tester,
  ) async {
    final container = await open(tester, const Size(1100, 900));
    final next = find.byKey(const ValueKey('skill-node-maneuver:man_next'));
    expect(
      find.descendant(of: next, matching: find.text('Gesperrt')),
      findsOneWidget,
    );
    await tester.tap(
      find.byKey(const ValueKey('skill-node-maneuver:man_base')),
    );
    await tester.pumpAndSettle();
    expect(find.text('Gezielter Kraftaufwand'), findsOneWidget);
    expect(
      find.text('Vollständiger Regeltext zum Wuchtschlag.'),
      findsOneWidget,
    );
    expect(find.text('Ein passendes Kampftalent'), findsWidgets);
    expect(find.text('Weit verbreitet'), findsOneWidget);
    expect(find.text('Testregelwerk'), findsOneWidget);
    ...
    expect(session.preview.combatConfig.specialRules.activeManeuvers, [
      'man_base',
    ]);
    expect(
      find.descendant(of: next, matching: find.text('Erlernbar')),
      findsOneWidget,
    );
    final repo = container.read(heroRepositoryProvider);
    expect(
      (await repo.loadHeroById(_hero.id))!
          .combatConfig
          .specialRules
          .activeManeuvers,
      isEmpty,
  ...
  testWidgets('Schmaler Baum erhält Vorstufen bei Suche und öffnet Details', (
    tester,
  ) async {
